...
def filter_tasks_not_in_db(tasks: List[TrackTask]) -> List[TrackTask]:
    """Filtriraj već postojeće trackove iz baze preko tracks.spotify_id.

    Ako baza ne postoji ili nema tablice/stupca, vraća originalnu listu.
    """
    if not tasks:
        return tasks

    if config is None or not hasattr(config, "get_main_db_path"):
        return tasks

    db_path = config.get_main_db_path()
    try:
        conn = sqlite3.connect(str(db_path))
    except Exception as e:  # pragma: no cover - defensive
        log.warning("Ne mogu se spojiti na bazu %s (%s) – ne filtriram.", db_path, e)
        return tasks

    try:
        cur = conn.execute("SELECT spotify_id FROM tracks WHERE spotify_id IS NOT NULL")
        existing_ids = {row[0] for row in cur if row[0]}
    except Exception as e:  # pragma: no cover - defensive
        log.warning(
            "Ne mogu čitati tracks.spotify_id iz baze %s (%s) – ne filtriram.",
            db_path,
            e,
        )
        conn.close()
    ...
    filtered: List[TrackTask] = [t for t in tasks if t.spotify_id not in existing_ids]
    skipped = len(tasks) - len(filtered)

    if skipped > 0:
        log.info(
            "U bazi već postoji %d od %d trackova; u batch ide %d novih.",
            skipped,
            len(tasks),
            len(filtered),
        )
        ...
        log.info(
            "Nijedan od %d trackova nije pronađen u bazi – svi idu u batch.",
            len(tasks),
        )
...
def get_default_batch_dir() -> Path:
    """Vrati default batch direktorij za downloader batch JSON-ove."""
    if config is not None and hasattr(config, "get_downloader_batch_dir"):
        try:
            return Path(config.get_downloader_batch_dir())
...
def _resolve_token_path() -> Path:
    ...
    # Minimalni fallback: isto ime fajla u rootu projekta
...
def load_spotify_token() -> Dict[str, Any]:
    ...
    if not token_path.is_file():
        raise SystemExit(
            "Nisam našao Spotify token JSON. Pokreni prvo spotify_oauth.py.\n"
...
def search_artist_by_name(name: str) -> Dict[str, Any]:
    """Nađi artista po imenu, ali robustno.

    - koristi Spotify search sa queryjem `artist:<name>`
    - prvo traži EXACT match po imenu (case-insensitive)
    - ako ima više exact match-eva, uzima onog s najvećom popularnošću
    - ako nema exact match, pada natrag na prvi rezultat od Spotifyja
    """
    query = f"artist:{name}"
    data = spotify_get(
        "/search",
    ...
    if exact:
        artist = max(exact, key=lambda a: a.get("popularity", 0))
        log.info(
            "Artist match (EXACT): %s (ID=%s, popularity=%s)",
            artist.get("name"),
            artist.get("id"),
        ...
        artist = items[0]
        log.info(
            "Artist match (FALLBACK 1st): %s (ID=%s, popularity=%s)",
            artist.get("name"),
            artist.get("id"),
            artist.get("popularity"),
        )
...
def choose_album_by_name(
    ...
    """
    album_name_norm = (album_name or "").strip().lower()
...
def handle_collection(args: argparse.Namespace) -> None:
    ...
    tasks = filter_tasks_not_in_db(tasks)
    if not tasks:
        log.info(
            "Sve tražene pjesme za ovaj upit već su u bazi; batch JSON se ne kreira."
        )
        return
